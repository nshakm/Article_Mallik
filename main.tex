...
Previous studies, such as \cite{howard_giant_2025}, have shown how the choice of EoS has an important impact on thermal evolution and the inference of planetary properties. \texttt{GASTLI} uses the following EoS: SESAME \citep{lyon_sesame_1992} for silicate in the core, CD21 \citep{chabrier_new_2021} for H/He in the envelope, M19 \citep{mazevet_ab_2019} \& AQUA \citep{haldemann_aqua_2020} for pure water in the envelope and atmosphere.

Thermal evolution sequences were generated with \texttt{GASTLI}. These results were used to generate a grid with dimensions of core mass fraction $\text{CMF}$, mass $M_p\: [M_J]$, equilibrium temperature $T_{\text{eq}}\: [\si{\kelvin}]$ and 
% internal temperature $T_{\text{int}}\: [\si{\kelvin}]$. 
age $\tau_p\: [\si{\giga\year}]$.
...
\clearpage
  ...
  \begin{figure*}[!t]
      ...
      \caption{Probability distributions of inferred bulk metallicities of four planets from the catalogue. The curves representing results of \texttt{GASTLI} and \texttt{planetsynth} are obtained using Gaussian KDEs in blue and orange respectively. \texttt{CEPAM} results are represented by the dashed-dotted line.}
...
Different tools used for interior structure modelling and thermal evolution are used for this comparison. They are present in the Tab.~\ref{tab:model_parameters}.
...
\begin{table*}
    ...
    \hline
    % \hline
    \textbf{Study}        & \textbf{Tool Name}    & \textbf{H/He EoS}            & $\mathbf{Z_{\text{env}}}$    & \textbf{Atmospheric Model} \\
    \hline
    \hline
    M26          & GASTLI       & CD21                & $Z_{\odot} = 0.014$ & petitCODE          \\
    MH25 \citep{muller_bulk_2025}         & planetsynth  & CD21                & $Z_{\odot} = 0.012$\footnote{Follows different convention for solar value} & Non-gray           \\
    P25 \citep{peerani_how_2025}          & CEPAM        & CMS19+HG23          & 0                   & Parmentier         \\
    % T19 \citep{teske_metal-rich_2019}        & ?            & ?                   & TODO                & TODO               \\
    \hline
    \end{tabular}
    \caption{TODO}
